...
def histogram_csv(
    input_path: Path = typer.Argument(..., help="input path"),
    output_path: Path = typer.Argument(..., help="output path"),
    column_names: List[str] = typer.Argument(
        ..., help="column to plot"
    ),
    ...
    my_colors = [(x/10.0, x/20.0, 0.75) for x in range(len(column_names))] # <-- Quick gradient example along the Red/Green dimensions.

    for i, column_name in enumerate(column_names):
        if len(bins) == 1:
            print(bins)
            df[column_name].hist(bins=bins[0], grid=False, figsize=(12,8), alpha=0.5, label=column_name)
        else:
            df[f"{column_name}_bins"] = pd.cut(df[column_name], bins)
            df[f"{column_name}_bins"].value_counts().plot.bar(alpha=0.5, label=column_name, figsize=(12,8), color=my_colors[i])
...
def bar_csv(
    ...
    df = pd.read_csv(input_path)
    for i, column_name in enumerate(column_names):
            df[column_name].value_counts().plot.bar(alpha=0.5, label=column_name, figsize=(12,8))
