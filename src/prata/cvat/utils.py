...
def get_all_task(project_id):
    req = f"{server}/api/projects/{project_id}/tasks?page_size=10000"
...
def create_tasks(
    ...
    for datapath in natsorted(list_data):
        ...
        task_name = (
            filename
            if task_name_with_parent is False
            else f"{datapath.parent.name}_{filename}"
        )
        uploaded_ids, uploaded_tasks = get_all_task(project_id)
        if task_name in uploaded_tasks:
            print(f"Skip {task_name} cause it's existed")
            continue
        data = {
            "name": task_name,
            "image_quality": 100,
        ...
        file = {
            ...
            "Content-Type": "application/zip" if ext == ".zip" else "video/mp4",
        }
        ...
        while True:
            response = requests.get(
                f"{server}/api/tasks/{task_id}/status", auth=auth
            )
            time.sleep(0.5)
            print(
        ...
        if annotation_dir is not None:
            ...
            annotation_path = (
                annotation_dir / filename / "annotations/instances_default.json"
            )
