...
def push_task(
    ...
    task_name_with_parent: bool = typer.Option(
        False, help="Task name will be concatenation of parent folder + filename"
    ),
):
    tmp = datapath
    zip_files = list(tmp.glob("**/*.zip"))
    mp4_files = list(tmp.glob("**/*.mp4"))
    files = zip_files + mp4_files
