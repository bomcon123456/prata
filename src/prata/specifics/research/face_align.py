...
def image_align_68(image, face_landmarks, face_landmarks_5pts, output_size=1024, transform_size=4096, enable_padding=True):
    # Align function from FFHQ dataset pre-processing step
    # https://github.com/NVlabs/ffhq-dataset/blob/master/download_ffhq.py
    np.random.seed(12345)
    bbox = cropByInputLM(image, face_landmarks, rescale=[1.4255, 2.0591, 1.6423, 1.3087])
    ...
    target = np.array([(0,0), (0, transform_size), (transform_size, transform_size), (transform_size, 0)], np.float32)
    ...
    img_np = np.array(img)
    return img_np, lm, lm_5pts


def image_align_5(image, face_landmarks, face_landmarks_5pts, output_size=1024, transform_size=4096, enable_padding=True):
    # Align function from FFHQ dataset pre-processing step
    # https://github.com/NVlabs/ffhq-dataset/blob/master/download_ffhq.py
    np.random.seed(12345)
    bbox = cropByInputLM(image, face_landmarks, rescale=[1.4255, 2.0591, 1.6423, 1.3087])
