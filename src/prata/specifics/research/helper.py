...
def parsevfhqtxt(txtfile, outputpath):
    NCOLS = 16
    with open(txtfile.as_posix(), "r") as f:
        lines = f.readlines()
    lines = lines[7:-1]
    lines = list(map(lambda x: x.strip(), lines))
    res = {}
    for line in lines:
        numbers = list(map(lambda x: float(x), line.split("\s+")))
        frameid, idx, x1, y1, x2, y2 = numbers[:6]
        if len(numbers) == NCOLS:
            lmks = numbers[6:]
        else:
            lmks = None
        d = dict(idx=idx, x1=x1, y1=y1, x2=x2, y2=y2, lmks=lmks)
        res[frameid] = d
    with open(outputpath.as_posix(), "w") as f:
        json.dump(res, f)


def parseline_vfhq(line):
    NCOLS = 16
    numbers = line.split()
...
def parseline_synergy(line):
    ...
    if len(numbers) == NCOLS:
        ...
        # lmks5pts = None
        # offset lmk5pts
        pose = numbers[16 - 10 : 19 - 10]
        lmks68pts = numbers[19 - 10 :]
    d = dict(
        frameid=frameid,
        idx=idx,
        x1=x1,
...
def mergetxt(gttxt, synergytxt, poseanhtxt, iqatxt):
    with open(gttxt, "r") as f:
        gtlines = f.readlines()[7:-1]
    with open(synergytxt, "r") as f:
        synergylines = f.readlines()[7:-1]
    with open(poseanhtxt, "r") as f:
        poseanhlines = f.readlines()[7:-1]
    with open(iqatxt, "r") as f:
...
if __name__ == "__main__":
    ...
    # df= merge3posedf(vfhq, synergy, poseanh)
    # print(len(df))
    mergetxt(
        Path("/home/termanteus/workspace/common/data_scripts/vfhqannot.txt"),
        Path("/home/termanteus/workspace/common/data_scripts/synergy.txt"),
        Path("/home/termanteus/workspace/common/data_scripts/poseanh.txt"),
        Path("/home/termanteus/workspace/common/data_scripts/iqa.txt"),
    )
