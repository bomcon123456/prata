...
def main(
    ...
    dry: bool = typer.Option(False, help="batchsize"),
):
    cachepath = Path(f"~/.cache/trungdt21/{imagedir.stem}_filelist.pkl")
    if cachepath.exists():
        with open(cachepath, "rb") as f:
            imagefiles = pickle.load(f)
    else:
        cachepath.parent.mkdir(exist_ok=True, parents=True)
        imagefiles = imagedir.rglob("*.png")
        imagefiles = natsorted(
            list(filter(lambda x: "mirror" not in x.resolve().as_posix(), imagefiles))
        )
    ...
    for chunk in tqdm(chunks):
        ...
        images = [
            Image.open(imagefile.as_posix()).convert("RGB") for imagefile in chunk
        ]
        inputs = processor(images, text=prompt, return_tensors="pt").to(
