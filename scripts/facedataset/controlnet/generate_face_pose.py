...
from mediapipeface import generate_annotation
...
def main(
    ...
    if os.path.exists(status_filename):
        ...
        status.pending = list(
            map(lambda x: x.resolve().as_posix(), input_path.rglob("*.png"))
        )
        # Output label file:
    ...
    with tqdm(total=len(status.pending)) as pbar:
        while len(status.pending) > 0:
            ...
            if "bad" in full_filename or "mirror" in full_filename:
                continue
            ...
            full_filepath = Path(full_filename)
            fname = full_filepath.name
            ...
            if annotated_output_directory:
                ...
                ) / full_filepath.relative_to(input_glob)
                annotation_filename.parent.mkdir(exist_ok=True, parents=True)
                annotation_filename = annotation_filename.as_posix()
            # output_filename = os.path.join(output_directory, fname)
            output_filename = Path(output_directory) / full_filepath.relative_to(
                input_glob
            )
            ...
            if "NSFW" in image_metadata:
                nsfw_marker = image_metadata.get(
                    "NSFW"
                )  # This can be "", None, or other weird things.
            ...
            (
                empty,
                annotated,
                faces_before_filtering,
                faces_after_filtering,
            ) = generate_annotation(
                img,
                max_faces=5,
    ...
    print(
        f"{len(status.skipped_smallface)} images rejected for having faces too small."
    )
